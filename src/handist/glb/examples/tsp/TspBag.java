...
public class TspBag implements Bag<TspBag, TspResult>, Serializable {
  ...
  public static void main(String[] args) {
    ...
    try {
      if (args.length == 2) {
        ...
        System.out
            .println("\t<PATH TO PROBLEM FILE> [subset of cities to use]");
        return;
      }
   ...
   * has `N+1` hops left to go where N is the index in {@link #BOUND_FUNCTION}.
   */
   ...
   * nextNodes[0][low[0], provided low[0] $lt; high[0].
   */
  int low[];
  ...
  /**
   * Path array, keeps track of which city the exploration is currently located
   * at. Its value at index 0 is always 0 as we arbitrarily decided that all
   * round trips start (and finish) at 0.
   */
  byte path[];
  ...
  /**
   * Constructor for the {@link TspBag} instance used to contain only the queue
   * of partial explorations.
   */
  protected TspBag() {
    BOUND_FUNCTION = null;
    ADJ_MATRIX = null;
  ...
  void exploreOne(TspResult shared) {
    ...
    if (candidates.length == 0) {
      ...
      if (totalCost <= shared.bestSolution) {
        ...
        for (int j = 0, i = index - TOTAL_NB_CITIES; i < index; i++, j++) {
  ...
  /**
   * Counts the number of leaves left unexplored in the exploration tree at the
   * current level
   *
  ...
  /**
   * The splitting strategy consists in giving away half of the work reserve if
   * it contains anything. If the work reserve is empty, the partial exploration
   * contained by this instance is split.
   *
   * @see #splitTheReserve()
