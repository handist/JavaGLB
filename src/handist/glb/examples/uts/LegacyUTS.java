...
public class LegacyUTS implements Serializable, Bag<LegacyUTS, Sum> {
   ...
   * number s (32 bit - 4 bytes integer) in the next 4 cells (from indices 16 to
   * 19 included), updates the message digest to use the beginning of array hash
   * on these cells and calls the parser to generate the random hash
   *
   * @param md
   ...
   * {@link #main(String[])} method. Selects the substring situated between the
   * specified bounds and returns it. It case the 'end' bound is greater than
   * the actual length of the string, the end of the string will be used
   * instead.
   *
   * @param str
   ...
   *
   * @param n
   *          depth of the exploration that will be possible to perform without
   *          increasing the size of the various arrays used in the
