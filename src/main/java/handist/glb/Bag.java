...
public interface Bag<B extends Bag<B, R> & Serializable, R extends Fold<R> & Serializable> {
     ...
     * without depleting the instance of all its work. If this bag is empty,
     * this method should also return {@code false}.
     *
     ...
     * @param sharedObject
     *            instance shared between workers of one place through which
     *            some data between workers can be shared
     */
    public void process(int workAmount, R sharedObject);

    /**
     * Takes a chunk of computation from this bag and returns it in a new
     * instance. The computation fragment this method returns will be computed
     * by a different thread, possibly on a different host.
     * <p>
