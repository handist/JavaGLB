...
public class GLBcomputer extends PlaceLocalObject {
    ...
    /**
     * Lock used to periodically call the method that will spread the
     * information contained in the shared memory.
     */
     ...
     * "i"th worker is requested to send work to the {@link #interPlaceQueue}.
     * The whole array is turned to 1 values when it is discovered by a worker
     * that the {@link #interPlaceQueue} is empty. Workers put their assigned
     * flag back to 0 when they feed the {@link #interPlaceQueue} as part of
     * their main routine {@link #workerProcess(WorkerBag)}.
     */
    ...
    /**
     * Lock used by the {@link #lifelineAnswerThread()} to yield its thread.
     * When a lifeline answer becomes possible, a
     * {@link #workerProcess(WorkerBag)} will {@link Lock#unblock()} this lock
     * to allow progress of the {@link #lifelineAnswerThread()}.
     */
     ...
     * lifeline answer thread is woken up for exit. When the lifeline answer
     * calls proceeds to method {@link #run(Bag)}, it can put {@link #shutdown}
     * back to {@code false} before the old thread could exit, resulting in
     * multiple lifeline answer thread running on the same place.
     * <p>
     ...
     * on other places.
     * <p>
     * The key is the integer identifying the remote place on which this place
     * may establish lifelines, the value is {@code true} when the lifeline is
     ...
     * work from this place but could not be given some work straight away as
     * part of method {@link #steal(int)}. The thieves stored in this member
     * will be answered by the thread running the
     ...
     * <p>
     * Worker processes check the value of this boolean before considering
     * waking the lifeline answer thread. This avoids having multiple workers
    ...
    /**
     * Logger instance used to log the runtime of the {@link GLBcomputer}
     * instance located at place {@link #HOME}.
     */
    PlaceLogger logger;
     ...
     * place to keep all the asynchronous tasks submitted to a place's runtime.
     * If an other class were to be used, errors when initializing this field in
     * the constructor of {@link GLBcomputer} are likely to appear.
     *
    ...
    /**
     * Instance in which the result of the computation performed at this place
     * is going to be stored. It is initialized with the given neutral element
     * before the computation starts in method
     ...
     * shutdown. Used when the place runs out of work and has established all
     * its lifelines.
     *
     * @see #run(Bag)
     */
     ...
     * parameters during the computation. Is given to the {@link #logger}
     * instance as the {@link PlaceLogger} class is in charge of calling the
     * tuning method.
     */
    final Tuner tuner;
    ...
    /**
     * Concurrent data structure for worker processes trying to yield. Each
     * worker must poll an available lock from this data structure before using
     * it. This avoids having concurrent workers attempting to yield using the
     * same lock. In practice, the current implementation has a single lock:
     * {@link #workerLock}, allowing a single worker to yield at the time.
     */
    ConcurrentLinkedQueue<Lock> workerAvailableLocks;

    /**
     * Collection of the {@link GLBcomputer.WorkerBag} of the inactive worker
     * threads on this place.
     * <p>
     * <em>Before the computation</em>, as many empty
     * {@link GLBcomputer.WorkerBag}s as concurrent workers
     * ({@link Configuration#x}) are placed in this collection as part of method
     * {@link #reset(SerializableSupplier, SerializableSupplier,SerializableSupplier, SerializableSupplier)}.
     * <p>
     * <em>During the computation</em>, this collection contains the
     * {@link Bag}s of the workers that are not active. Prior to launching a new
     * asynchronous {@link #workerProcess(WorkerBag)}, a
     * {@link GLBcomputer.WorkerBag} is polled from this collection and some
     * computation is merged into it. When a {@link #workerProcess(WorkerBag)}
     * terminates because it has completed its fraction of the computation and
     * could not get more work from the intra place load balancing mechanisms,
     * it places its {@link GLBcomputer.WorkerBag} back into this collection.
     * <p>
     ...
     * protected manner. This includes segments of methods
     * {@link #deal(int, Bag)} and
     * {@link #workerProcess(GLBcomputer.WorkerBag)}.
     * <p>
     ...
     * collection of each computation fragment in method
     * {@link #collectResult()}.
     */
    ConcurrentLinkedQueue<WorkerBag> workerBags;

    /**
     * Keep tracks of the number of {@link #workerProcess(WorkerBag)} launched
     * on this place. Access is protected by synchronized blocks with
     * {@link #workerBags} as lock provider.
     * <p>
     ...
     * launched. Moreover, the {@link #workerProcess(WorkerBag)} can
     * cooperatively yield its thread usage to allow some other asynchronous
     * activities to be performed. Those yields do not change the value of
     * {@link #workerCount} but are tracked separately in class member
     * {@link #logger} with methods {@link PlaceLogger#workerYieldStart()} and
     ...
     * <p>
     * The constructor is kept hidden as some specific setup needs to be done
     * for the distribution to take place. The class
     * {@link GLBfactory#setupGLB()} is the method the programmer should use to
     * get a computation service instance.
     *
     * @param workUnit
     *            amount of work processed by workers before interrupting their
     *            routine
     * @param randomSteals
     *            maximum number of steals this place can perform before turning
     *            to its lifelines to steal some work
     * @param s
     ...
     * @param tuningTimeout
     *            number of nanoseconds that need to elapse between two calls to
     *            the tuning class
     * @param t
     *            implementation of interface {@link Tuner} in charge of
     *            adjusting the parameters used by the GLB during the
     *            computation.
    ...
    GLBcomputer(int workUnit, int randomSteals, LifelineStrategy s,
        ...
        CONFIGURATION = new Configuration(places().size(),
                maximumConcurrentWorkers, workUnit, randomSteals,
                s.getClass().toString(), tuningTimeout, tunerClass,
                whisperInterval);
     ...
     * member {@link #result}. If this place ({@link #HOME}) is not place 0,
     * sends the content of the result to place 0 to be merged with all the
     * other results there.
     */
    @SuppressWarnings({ "rawtypes", "unchecked" })
    void collectResult() {
        synchronized (result) { // Synchronized in case this is place 0 and
                                // remote
    ...
    void communicatorThread() {
        while (!communicatorThreadShutdown) {
            ...
            communicatorLock
                    .setNextWakeup(lastCall + CONFIGURATION.whisperInterval);
            try {
    ...
    public <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> R compute(
        ...
        return compute(bag, initResultSupplier, emptyBagSupplier,
                emptyBagSupplier);
    }
    ...
    /**
     * Launches the computation with a periodic whisper based information
     * sharing between the distributed processes. This method allows for
     * differentiated implementations of the splitting methods between the
     * workers and the queues used at each place for load balancing.
     *
     * @param <R>
     *            Type of the result produced by the computation
     * @param <B>
     ...
     * @param <W>
     *            Type in charge of performing the communication between the
     *            hosts
     * @param work
     ...
     *            initializer for the queues used to balance the work between
     *            the workers and between processes
     * @param emptyBagSupplier
    ...
    /**
     * Launches the computation with a periodic whisper based information
     * sharing between the distributed processes.
     *
     * @param <R>
     *            Type of the result produced by the computation
     * @param <B>
     ...
     * stealing from random victims or stealing from lifelines. This will cause
     * it to resume computation by spawning a first worker (with the merged
     * loot) as part of the {@link #run(Bag)} routine.
     * <li>If the place is inactive, method {@link #run(Bag)} is launched with
     * the loot as parameter.
     * </ul>
     *
     * @param victim
    ...
    void deal(int victim, Bag loot) {
        ...
        synchronized (workerBags) {
            ...
            case 0:
                ...
                return; // Placing this return instruction allows us to put the
                        // run call
                        // out of the synchronized block without having to use
                        // an extra
                        // condition.
            ...
            case -1:
                ...
                /*
                 * If the place is currently stealing, the bag is given to the
                 * head of collection workerBags. This head is the one which is
                 * going to be run when the stealing stops and a new
                 * workerProcess is spawned in method run
                 */
            ...
            case -2:
                ...
                workerCount = 1;
                state = 0; // Possible concurrent lifeline answers will not
                           // spawn
                           // a new run method as this signals that this place
                           // is
                           // now "alive"
            }
        }
    ...
    /**
     * Activity spawned by method {@link #run(Bag)} to answer lifelines that
     * were not able to be answered straight away.
     * <p>
     * This process yields until a lifeline answer is signaled as possible or
     * the <em>shutdown</em> is activated by method {@link #run(Bag)}.
     */
    @SuppressWarnings("rawtypes")
    void lifelineAnswerThread() {
        ...
        do {
            ...
            try {
                ...
                // Should not happen since the implementation of Lock does not
                // throw
     ...
     * {@link #performLifelineSteals()}. Gets some loot from the inter/intra
     * place queues and performs the status updates on these queues as
     * necessary.
     *
        ...
        if (!interQueueEmpty) {
            synchronized (intraPlaceQueue) {
                if (!interQueueEmpty) {
                    ...
                    // intraQueueEmpty = intraPlaceQueue.isEmpty(); // Update
                    // flag
                    // }
                    loot = interPlaceQueue.split(true);
                    logger.interQueueSplit.incrementAndGet();
    ...
    boolean performLifelineSteals() {
        for (int i = 0; i < LIFELINE.length; i++) {
            ...
            if (!lifelineEstablished.get(lifeline)) { // We check if the
                                                      // lifeline was
            ...
            synchronized (this) {
                try {
                    ...
                    // If an Interrupted exception is thrown, it does not hurt
                    // the program
            ...
            synchronized (workerBags) {
                if (state == 0) { // State is put back to 0 in lifelineDeal when
                                  // an
                                  // answer is received
                    return true;
                } else if (i == LIFELINE.length - 1) {
                    // If all lifelines were established and still no positive
                    // answer was
                    // received
                    state = -2;
                }
     ...
     * <ul>
     * <li>Some work is received by this place, either by a previously
     * established lifeline or through a random steal initiated by this method
     * <li>The maximum number of steals on a random place is reached
     * </ul>
     * These two events are not mutually exclusive, it can happen that the
     * maximum number of random steals was reached and that some work was
     * received by this place concurrently.
     *
     * @return {@code true} if some work is received during the method's
     *         execution, {@code false} otherwise
     */
    boolean performRandomSteals() {
        ...
        for (int i = 0; i < CONFIGURATION.w; i++) {
            ...
            // Checks if some work was received
            synchronized (workerBags) {
                if (state == 0) { // State is put back to 0 when an answer is
                                  // received
                    return true;
                }
            }
     ...
     * {@link #interPlaceQueue} is empty. This will make the workers send part
     * of their work this queue.
     */
     ...
     *
     * @param <R>
     *            type parameter of the result the computation to come will
     *            produce
     * @param <B>
     *            type parameter of the computation
     * @param <W>
     *            type of the whisperer in charge of propagating shared
    ...
    <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable, W extends Whisperer<? extends Serializable, R>> void reset(
        ...
        // We establish lifelines on this place for initial work-stealing
        // conditions
        ...
        if (whispererSupplier != null) {
            ...
            whisperer = null;
            communicatorThreadShutdown = true; // Not necessary as this member
                                               // will
                                               // not be used if there is no
                                               // whisperer. However, setting
                                               // the
                                               // value to true maintains a
                                               // certain
                                               // consistency
        }
     ...
     * <p>
     * Spawns the first worker thread (which will in turn recursively spawn
     * other worker threads). When all workers run out of work, attempts to
     * steal work from remote hosts.
     *
     * @param b
    ...
    void run(Bag b) {
        ...
        do {
            do {
                ...
                // All the workers have stopped, this place does not have any
                // work and
    ...
    /**
     * May be called by a tuner to spuriously launch the feeding of the
     * intra-bag
     */
    ...
    /**
     * Launches an distributed warm-up on each process in the distributed
     * cluster.
     * <p>
     ...
     * launched. This method launches the provided computation on each host of
     * the cluster. No load balance between the compute nodes in the cluster is
     ...
     *            supplier of the work sample which will be computed at each
     *            place independently as a warm-up
     * @param resultInitializer
     *            initializer for the result instance of each place
     * @param queueInitializer
     *            initializer for the two queues used at each place to perform
     *            load balancing operations
     * @param workerInitializer
     *            initializer for the bags held by each worker
     * @return logger instance containing information about the warm-up
     *         execution
     */
    public <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> Logger warmup(
        ...
        finish(() -> {
            for (final Place p : places()) {
                asyncAt(p, () -> {
                    reset(resultInitializer, queueInitializer,
                            workerInitializer, null);
     ...
     * currently processing can be split and the {@link Configuration#x} limit
     * was not reached (meaning there are {@link Bag} instances left in member
     * {@link #workerBags})
     ...
     * of active workers has reached the number of available cores on the
     * system, yields its execution to allow execution of other activities.
     * <li>Processes a chunk of its bag
     * <li>Repeat steps 1. to 6. until the {@link Bag} of which this worker is
     * in charge becomes empty.
     * <li>When the bag becomes empty as a result of splitting and processing
     * it, the worker attempts to get some more work from the
     * {@link #intraPlaceQueue} and the {@link #interPlaceQueue}. If successful
     * in acquiring some work, resume its routine from step 1. If unsuccessful,
     * stops operating.
     * </ol>
     *
     * @param workerBag
        ...
        for (;;) { // Infinite loop, not a mistake
            do {
                ...
                if (!workerBags.isEmpty() && bag.isSplittable()) {
                    ...
                    // polling of workerBags may yield null if a concurrent
                    // worker polled
                ...
                /*
                 * 2. Checking the status of the Bag used for intra place load
                 * balancing
                 */
                if (intraQueueEmpty) {
                    if (bag.isSplittable()) {
                ...
                if (!lifelineThieves.isEmpty() && !interQueueEmpty) {
                    logger.lifelineAnswerThreadHold();
                    lifelineAnswerLock.unblock(); // unblocking lifeline answer
                                                  // thread,
                ...
                if (workerCount == CONFIGURATION.x
                    ...
                    if (l != null) {
                        ...
                        try {
                            ...
                            // Should not happen in practice as the
                            // implementation Lock does
            ...
            } while (!bag.isEmpty());// 7. Repeat previous steps until the bag
                                     // becomes
                                     // empty.
             ...
             */
            synchronized (workerBags) { // Decision on whether this worker is
                                        // going to
                                        // continue is made here. This decision
                                        // needs
                                        // to be done in a synchronized block to
                                        // guarantee mutual exclusion with
                                        // method
                                        // lifelineDeal.
                ...
                if (!intraQueueEmpty) {
                    ...
                    synchronized (intraPlaceQueue) {
                        if (!intraQueueEmpty) {
                            loot = intraPlaceQueue.split(true); // If only a
                                                                // fragment
                                                                // can't
                                                                // be taken, we
                                                                // take the
                ...
                } else if (!interQueueEmpty) { // Couldn't steal from
                                               // intraQueue, try on
                                               // interQueue
                    Bag loot = null;
                    synchronized (intraPlaceQueue) {
                        if (!interQueueEmpty) {
                            loot = interPlaceQueue.split(true); // Take from
                                                                // interplace
                            logger.interQueueSplit.incrementAndGet();
                            // interQueueEmpty = interPlaceQueue.isEmpty(); //
                            // Update the flag
                            ...
                            /*
                             * if (loot.isSplittable()) { // Put some work back
                             * into the intra queue
                    ...
                    workerCount--;
                    if (workerCount == 0) {
                        state = -1; // No more workers, we are now in stealing
                                    // mode
                    }
                    logger.workerStopped();
                    workerLock.unblock(); // A yielding worker can be unlocked.
                                          // As this worker is terminating, its
                                          // thread
            ...
            // Stealing from the queues in the place was successful. The worker
            // goes
            // back to processing its fraction of the work.
            logger.workerResumed();

        } // Enclosing infinite for loop. Exit is done with the "return;" 7
          // lines
          // above.
    }
}
