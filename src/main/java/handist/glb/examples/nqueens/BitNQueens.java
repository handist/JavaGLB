...
public class BitNQueens implements Bag<BitNQueens, Answer>, Serializable {
    ...
    public static void main(String args[]) {
        ...
        System.out
                .println("Launching a sequential " + n + "-queens computation");
        ...
        System.out
                .println("Computation time (s);" + computationTime / 1e9 + ";");
    }
    ...
    /**
     * Array of the next option that remains to be attempted at each level of
    ...
    /**
     * Resets this instance to start the whole computation. For computation
     * using the GLB, you should call {@link #initParallel()} instead of this
     * method.
     */
    public void init() {
        depth = 0;
     ...
     *
     * @param c
     *            instance whose exploration is to be continued
     */
        ...
        if (reserve.size() > 1
            ...
            for (int i = 0; i <= depth; i++) {
                ...
                if (options >= 2) {
                    ...
                    newDepth = i; // There is at least one node left on this
                                  // level
                    lootDepth = i;
                } else if (options == 1) {
                    if (takeAll) {
                        treeUpperBound[i]--; // Makes it equal to
                                             // treeLowerBound[i];
                        lootDepth = i;
                    } else {
                        newDepth = i; // The single node remaining is not given
                                      // away
                    }
                }
            }
            ...
            depth = newDepth; // This makes the current instance backtrack to
                              // the last
    ...
    public void step() {
        ...
        if (treeLowerBound[depth] < treeUpperBound[depth]) {
            // We pick the next candidate and create the corresponding mask for
            // the
            // candidate queen
            final int mask = 1 << treeLowerBound[depth];
            treeLowerBound[depth]++; // This option at the current level is
                                     // explored
            nodesExplored[depth]++;
            ...
            if (0 == (mask & stackMask[depth])) {
                ...
                if (depth == N - 1) {
                    ...
                    // The candidate can be placed. We integrate it to the board
                    // and write
                    // it to the stack
                    stackColumn[depth + 1] = stackColumn[depth] | mask;
                    stackAntiDiagonal[depth
                            + 1] = (stackAntiDiagonal[depth] | mask) << 1;
                    stackDiagonal[depth
                            + 1] = (stackDiagonal[depth] | mask) >>> 1;
                    stackMask[depth + 1] = stackDiagonal[depth + 1]
                            | stackAntiDiagonal[depth + 1]
                            | stackColumn[depth + 1];

                    // We prepare the next level of exploration (allow all
                    // options)
                    depth++;
