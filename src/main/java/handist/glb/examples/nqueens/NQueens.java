...
public class NQueens implements Bag<NQueens, Answer>, Serializable {
    ...
    /**
     * Starts a N-Queens computation, N being the parameter to specify the size
     * of the problem to solve. With a second (optional) parameter, the user can
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < repetitions; i++) {
            ...
            System.out.println(i + "/" + repetitions + ";"
                    + problem.solutionCount + ";" + treeSize + ";" + init / 1e9
                    + ";" + comp / 1e9 + ";");
        }
    }
     ...
     * the various arrays used to describe the exploration. A value of
     * {@code -1} indicates that the exploration is finished.
     */
    int depth;
     ...
     * {@link #high}, it describes an interval of branches in the exploration
     * tree that remain to explore at each level.
     */
    int[] low;
    ...
    /**
     * Stack used to keep the id of the rows of the cover matrix used
     * (equivalent to a piece being placed on the board).
     */
    int[] stack;
    ...
    /**
     * Set this instance to be in a state at which it contains the whole
     * problem.
     */
    public void init() {
        depth = 0;
    ...
    /**
     * A N-Queens exploration is empty if the current exploration is finished
     * and if there are no partial exploration left in the {@link #reserve}.
     *
    ...
    /**
     * Indicates if this instance of N-Queens can be split, that is if work can
     * be taken from this instance without emptying it completely.
     *
     ...
     * away. If not, half of the leaf nodes of this current exploration are
     * given away.
     */
    @Override
            ...
            for (int level = 0; level <= depth; level++) {
                ...
                if (options > 1) {
                    ...
                    if (alternator) {
                        ...
                        high[level]--; // This removes the node from this
                                       // instance
                        loot.depth = level;

                        if (depth == level) {
                            // The deepest leaf of this instance was given away.
                            // We need to
    ...
    private void step() {
        if (choiceLeft() > 0) {
            ...
            if (!oldChoice.isHeader()) {
                // The check removes the case where it is actually the first
                // time we are
            ...
            if (matrix.hasHope()) {
                branchCount[depth]++;
                depth++; // It is still possible to find a solution, we continue
                         // the
                         // exploration
              ...
              // Choosing the considered row resulted in a column not capable of
              // being
              // covered later. The next possible row will be tested at the
              // current
     ...
     * exploration contains at least two leaf nodes, in which case the
     * exploration tree can be split.
     *
     * @return true if the current exploration tree can be split, false
     *         otherwise.
     */
