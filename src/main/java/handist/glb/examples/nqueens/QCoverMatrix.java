...
public class QCoverMatrix {
    ...
    /**
     * Index at which the columns that represent the diagonals and
     * anti-diagonals in the board start. As it is not possible to cover all the
     * diagonals and anti-diagonals of the chess board in the N-Queens problem,
     * the corresponding columns in the cover matrix are allowed to remain
     * uncovered. As a result, only the columns of the matrix that correspond to
     * the ranks and files of the board need to be covered.
     *
     * @see #hasHope()
     */
     ...
     * must indicate the index of the column in which a cell must be created.
     * The indices in the arrays should be sorted by increasing number.
     *
     * @param columnNames
     *            array containing the names of each individual column of the
     *            matrix
     * @param columnCheckLimit
     *            indicates that only the n'th first columns in the matrix
     *            absolutely have to be covered ; setting the value to the
     *            length of array columnNames will enforce solutions to the
     *            matrix cover all columns
     *
     * @param rows
    ...
    /**
     * Checks if the size of the remaining columns are consistent with the
     * number of cells in the columns linked lists. Any discrepancy is printed
    ...
    /**
     * Indicates if there is hope of finding a solution to the matrix. Checks
     * that every column has at least one row still under it. If one column is
     * found to have no row that can cover it, returns false. Otherwise returns
     * true.
     * <p>
     * Note that in this special implementation dedicated to the N-Queens
     * problem, only the matrix columns corresponding to the rows and columns of
     * the board are checked. Matrix columns for the diagonals and
     * anti-diagonals are not checked as they are considered "additional
     * constraints" and solutions to the N-Queens problem cannot by nature cover
     * all of the (anti-)diagonals.
     *
