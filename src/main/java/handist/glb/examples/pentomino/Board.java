...
public class Board {
    ...
    /**
     * Initializes a board with the given dimensions. The product of the width
     * and height arguments should be equal to 60 or 90.
     *
     * @param w
    ...
    public boolean placePiece(Piece p, int variation) {
        ...
        final int shift = toPlace[0]; // the top left tile of the piece may not
                                      // be
                                      // described with 0, in which case the
                                      // whole
                                      // piece needs to be translated towards
                                      // the
                                      // left.
        for (int i = 0; i < toPlace.length; i++) {
            ...
            if (board[tileIndex] != EMPTY) {
                // The piece cannot be placed here, restore the board and return
                // false
                for (int j = i - 1; 0 <= j; j--) {
