...
public class Pentomino implements Bag<Pentomino, Answer>, Serializable {
    ...
    enum PentominoType {
        ...
        /**
         * 18 pieces Pentomino with the face-down variations of the chiral
         * pieces as pieces of their own with a 90 tile board
         */
        ONE_SIDED
    };
     ...
     * at which a piece is placed on the board (only relevant if variation is
     * not -1). Instances of this class are used in member {@link #placement} to
    ...
    /**
     * Launches a sequential Pentomino exploration with the board of the
     * specified width and height. The third argument specifies if the
     * symmetries of the problem should be removed from the exploration. Whether
     * the exploration is a regular Pentomino of the one-sided variant is
     * determined based on the dimensions of the board.
     *
     * @param args
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            System.err.println(
                    "Error parsing arguments <W> <H> <symmetriesRemoval>");
            return;
        }
        ...
        if (args.length > 3) {
            // Parse the additional arguments determining the positions of the X
            // piece
            // to be tried
     ...
     * pentomino algorithm. If we arbitrarily place pieceX at the beginning of
     * the computation, the total number of pieces to place is reduced from 12
     * to 11 (or 18 to 17).
     */
    ...
    /**
     * PieceP instance which is contained in {@link #pieces} but kept as a
     * member to allow easy access in case further symmetry restrictions need to
     * be applied to the Pentomino problem
     */
    transient PieceP P;
    ...
    /**
     * Indicates of the current instance needs additional restrictions on
     * symmetry In the case of {@literal PentominoType#ONE_SIDED}, value 0
     * indicates no need for extra symmetry, -1 indicates need for removal of
     * horizontal symmetries and 1 indicates need for removal of vertical
     * symmetries.
     */
     ...
     * <p>
     * The reason why this member and the {@link #pieces} member are not kept in
     * a single data structure is that when transferring work we should not copy
     * the pieces as they remain the same throughout the computation. The only
     * relevant information is how they are placed on the board.
     */
    PiecePlaced[] placement;

    /**
     * Stack containing the indices of the pieces in array pieces that were
     * placed on the board
     */
    int[] stack;

    /**
     * Indicates that 'depth' pieces have been chosen/orientated and placed on
     * the board. Used as index for arrays {@link #low}, {@link #high}, and
     * {@link #stack}.
     */
    int depth;
    ...
    /**
     * Private constructor which does not initialize any member, Used for
     * creating the only necessary members when splitting the Pentomino problem
     *
     * @param t
     ...
     * {@link #pieces} of the next piece which the exploration is going to try
     * to place. As a side effect, it also sets the variation that is going to
     * be tried in the appropriate {@link PiecePlaced} instance.
     *
     * @param number
    ...
    private int getRemaining(int number) {
        ...
        for (;; ppIndex++) {
            if (placement[ppIndex].index == -1) { // If the piece isn't placed
                ...
                if (vars > number) { // If the next piece to try is said piece,
                                     // break
                    break;
    ...
    private int getTotalOptions() {
        ...
        for (int i = 0; i < placement.length; i++) {
            ...
            if (pp.index < 0) {
                // Piece has not been placed yet, we count its different
                // positions
     ...
     *
     * @return a pentomino instance with the minimum number of members
     *         initialized
     */
     ...
     *            indicates which initial positions of piece X should be
     *            included in the search. This argument is ignored if the
     *            symmetries are not removed. If null, all possible initial
     *            positions of Piece X are included in the exploration.
     */
    public void init(PentominoType type, boolean noSymmetries,
        ...
        if (noSymmetries) {
            if (type == PentominoType.STANDARD) {
                ...
                for (int i = 0; i < (height - 1) / 2; i++) {
                    for (int j = 0; j < (width - 1) / 2; j++) {
                        ...
                        // Generate Pentomino instances with pieceX in (j,i)
                        // coordinates

                        final int placementIndex = i * (width + Board.SENTINEL)
                                + j;
                        if (placementIndex != 0) {
                            if (specificPositions == null
                                ...
                                p.high[0] = 1;
                                p.low[1] = 0;
                                p.high[1] = 62; // 63 different possibilities
                                                // minus the X only
                                                // orientation

                                // Remove additional symmetry in cases where
                                // PieceX is placed on
                                ...
                                if ((height % 2 == 1
                                    ...
                                    p.high[1] -= 4; // Removes 4 face down
                                                    // placement of piece P
                                }
                    ...
                    for (int j = 0; j < (width - 1) / 2; j++) {
                        ...
                        if (placementIndex != 0) {
                            if (specificPositions == null
                                ...
                                // Remove additional symmetry in cases where
                                // PieceX is placed on
                                // the center column or the center line, note
                                // that the actual
                                // symmetry removal operation is made when
                                // taking the
                                ...
                                if (height % 2 == 1
                                    ...
                                    p.additionalSymmetryRestriction = 1;
                                    p.high[1] -= 2;// Removes 2 orientation of
                                                   // the V piece
                                } else if ((width % 2 == 1
                                        && j + 1 == (width - 1) / 2)) {
            ...
            final Pentomino p = getInitPentomino();
            p.high[0] = 63; // Sum of all the possible orientations of all the
                            // pieces
                            // (is 63 for both the regular pentomino and the
                            // one-sided
                            // pentomino)
            p.depth = 0;
            p.additionalSymmetryRestriction = 0;
    ...
    /**
     * Initializes the {@link #pieces} array with the pieces that are relevant
     * to the specified pentomino problem (standard pentomino or one-sided
     * pentomino)
     *
     * @param type
    ...
    public void step() {
        if (optionsLeft() > 0) {
            ...
            if (board.placePiece(piece, var)) {
                treeBranch[depth]++;
                // place p has been placed, we remove it from the pieces left to
                // place
            ...
            // backtrack
            // remove the piece placed from the stack and make it available
            // again
            depth--;
     ...
     *
     * @return {@code true} if the exploration can be split in two,
