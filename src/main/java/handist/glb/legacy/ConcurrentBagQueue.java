...
class ConcurrentBagQueue<R extends Fold<R> & Serializable> {
   ...
   * existing instance, the {@link Bag} is added at the last available index in
   * the array. If the array becomes full as a consequence, its size is
   * increased.
   *
   * @param <B>
  ...
  /**
   * Indicates if this {@link ConcurrentBagQueue} is empty, that is that it does
   * not contain any {@link Bag} or all the {@link Bag}s it contains are empty.
   *
