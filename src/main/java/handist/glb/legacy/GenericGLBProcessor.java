...
final class GenericGLBProcessor extends PlaceLocalObject
     ...
     * {@link Bag} processed by this place contained in {@link #bagsToDo} when
     * the computation ends. All the remote instances are then merged back into
     * the instance of place 0.
     *
     * @see #result()
     */
    @SuppressWarnings("rawtypes")
    ...
    /**
     * Integer ({@code int}) id's of the places which are susceptible to
     * establish their lifeline on this place. There is no particular meaning to
     * the indices of the array in which this information is contained.
     */
    private final int incomingLifelines[];

    /**
     * Integer ({@code int}) id's of the places on which this place will
     * establish its lifelines when running out of work.There is no particular
     * meaning to the indices on which this information is contained.
     */
    private final int lifelines[];

    /**
     * Map indicating if the lifelines this place can establish are activated.
     * The key is the identifier of the remote place on which a lifeline can be
     * established, i.e. the keys are the content of array {@link #lifelines}.
     * The mapped boolean indicates if the lifeline is activated. For example if
     ...
     * places), we avoid having the same sequence of places to thieve from for
     * all the places.
     */
    private final Random random = new Random(home.id);

    /**
     * Number of random steal attempts performed by this place. Can be adjusted
     * to the user's convenience with the constructor.
     */
     ...
     * <em>p</em>
     * </ul>
     * At initialization, is in inactive state. Due to race conditions, it has
     * to be protected at each read/write.
     */
    private int state = -2;

    /**
     * List of thieves that asked for work while the current place was
     * performing computation. They will be answered in the
     * {@link #distribute()} method.
     */
     ...
     * state. Parameter {@code init} is kept in member {@link #result}. Having
     * the parameter type of the result also allows us to initialize a clean
     * instance of class {@link ConcurrentBagQueue} for member {@link #bagsToDo}
     * with the proper generic parameter type.
     *
     * @param <R>
     *            result parameter type
     * @param init
     *            neutral element of the desired result type for the computation
     *            to come
     */
     ...
     * {@link #bagsToDo} for any work and either process the given work or
     * switch to the lifeline steal procedure.
     *
     * @param <B>
    ...
    private synchronized <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> void deal(
        ...
        /*
         * If answering place couldn't share work with this place, the given q
         * is null. A check is therefore necessary.
         */
        if (gift != null) {
        ...
        /*
         * Whichever the outcome, the worker thread blocked in method run needs
         * to be waken up
         */
    ...
    private <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> void distribute() {
        ...
        while ((p = lifelineThieves.poll()) != null) {
            ...
            if (toGive != null) {
                ...
                /*
                 * null split means no more work to share in bagsToDo. We put
                 * the thief back into the collection
                 */
                lifelineThieves.add(p);

                /*
                 * All further calls to bagsToDo.split() would yield null at
                 * this stage, it is not worth continuing
                 */
                return;
            }
        }
     ...
     * {@link Bag#submit(Fold)}}, storing it into member {@link #result()}
     * before sending this result fragment to place 0 where it will be merged
     * with all the result fragments from every place in the computation.
     * <p>
     ...
     * call to this method originating from each of the lifeline, each giving
     * some work to this place. This is handled by the concurrency protection of
     ...
     * value of {@link #state} is necessary to ensure no two processes are
     * running method {@link #run()} at the same time.
     *
     * @param <B>
    ...
    private <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> void lifelineDeal(
        ...
        /*
         * Call to run needs to be done outside of the synchronized block.
         * Boolean toLaunch is used to carry the information outside the
         * synchronized block.
         */
     ...
     * effect.
     * <p>
     * If there is only one host in the computation cluster and that host ran
     * out of work, it means the computation has completed. There are no other
    ...
    /**
     * Method used to signal the fact that the {@link GenericGLBProcessor}
     * located at place {@code p} is requesting work from this place.
     * <p>
    ...
    private void request(Place p) {
        synchronized (this) {
            log.stealsReceived++;
            /*
             * If the place is currently performing computation, adds the thief
             * to its list of pending thief. The work will be shared when this
             * place stops processing its tasks in the main 'run' loop by the
    ...
    private void run() {
        ...
        /**
         * Sending null to thieves that have managed to ask for some work
         * between two random steals. This is absolutely necessary. The
         * computation may not end if this was not done.
         */
        Place p;
        while ((p = thieves.poll()) != null) {
            uncountedAsyncAt(p, () -> {
    ...
    private void steal() {
        ...
        if (p >= h.id) {
            // We cannot steal on ourselves. Moreover the generated random
            // integer has
        ...
        /*
         * Change state to 'p', i.e. thieving from place 'p' before requesting
         * work from it.
         */
        synchronized (this) {
            state = p;
         ...
         * 'uncounted' as this asynchronous call is about program "logistics"
         * and does not need to intervene in the enclosing "finish" construct
         */
    ...
    /**
     * Gives back the result that was gathered from the {@link Bag}s contained
     * in the {@link #bagsToDo} member of all the places. This method is called
     * in the {@link #compute(Bag, Fold)} method when it is detected that the
     ...
     *            number of random steals attempts before turning to the
     *            lifeline stealing scheme
     * @param s
