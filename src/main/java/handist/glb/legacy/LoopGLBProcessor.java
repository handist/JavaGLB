...
final class LoopGLBProcessor extends PlaceLocalObject implements GLBProcessor {
    ...
    /**
     * Fold instance for this local place. Is initialized with the neutral
     * element before a computation takes place in method {@link #reset(Fold)}.
     * When computing the result, the member is then passed to each bag for them
     * to add in their contribution ({@link Bag#submit(Fold)}).
     */
    @SuppressWarnings("rawtypes")
     ...
     * every place as waiting for work from its lifeline except place 0 since
     * this is the place which is going to receive all the work when the
     * computation start.
     *
     * @see #lifelineSteal()
     ...
     * performing computation. Each will be answered in the
     ...
     * before waking up the waiting thread in the {@link #steal()} procedure.
     * This will in turn make this place check its {@link #bagsToDo} for any
     * work and either process the given work or switch to the lifeline steal
     * procedure.
     *
     * @param <B>
    ...
    private synchronized <R extends Fold<R> & Serializable, B extends Bag<B, R> & Serializable> void deal(
            B gift) {
        // If the place from which the random steal is performed couldn't share
        // work
    ...
    /**
     * Computes the result from the bags processed at this place, storing it
     * into member {@link #result()} before sending those to place 0 if this
     * place isn't place 0.
     *
     * @param <R>
     *            result type
    ...
    private <R extends Fold<R> & Serializable> void gather() {
         ...
         * necessary for place 0 where the bags processed by place 0 submit
         * their result into the result member in this method while the other
         * places will send their member result for it to be folded into the
         * result member in method giveResult(). Incidentally, using member
         * result for the lock.
         */
        synchronized (result) {
     ...
     * in response to a {@link #lifelineSteal()} by the place giving the work.
     * The given work is merged into the {@link #bagsToDo} member and method
     ...
     * <p>
     * The lifeline strategy in this implementation consists in a single
     * directed loop linking all the nodes. That is:
     * <ul>
     * <li>place <em>n</em> establishes its lifeline on place <em>n-1</em></li>
     * <li>place <em>0</em> establishes its lifeline on place
     * <em>({@link #places} - 1)</em></li>
     * </ul>
     *
     * @see #lifeline
     */
    ...
    /**
     * Called when a place asks work from this instance, the place asking for
     * work is passed as parameter.
     * <p>
     ...
     * work in its {@link #bagsToDo} before answering to potential thieves
    ...
    private void run() {
        ...
        /**
         * At this stage, the place has no more work and its random steals
         * attempts have failed. It is now time to turn to the lifeline scheme.
         */
         ...
         * random steals. This is absolutely necessary. The computation may not
    ...
    private void steal() {
        ...
        if (p >= h.id) {
            // We cannot thief on ourselves. Moreover the generated random
            // integer has
        ...
        // Change state to 'p', i.e. thieving from p before requesting work from
        // it.
        synchronized (this) {
            state = p;
        }
        ...
        // does not need to intervene in the enclosing "finish" for the
        // computation.
        uncountedAsyncAt(place(p), () -> {
            request(h);
        });

        synchronized (this) { // synchronized block necessary for the wait()
                              // call.
            /*
             * Enclosing safety loop. State is set back to -1 ('running') in
             * method deal when the answer is received, thus lifting the
             * barrier.
             */
            while (state >= 0) {
                try {
    ...
    /**
     * Merges the given parameter into this instance {@link #result} member.
     * Only called on place 0. Mutual exclusion between several calls to this
     * method as well as {@link #gather()} method is ensured.
     *
     * @param <R>
     *            type of the result
     * @param res
     ...
     * in the {@link #bagsToDo} member of all the places. The computation needs
     * to be over before this method is called.
     *
     * @param <R>
     ...
     *            number of random steals attempts before resaulting to the
     *            lifeline thief scheme
     */
