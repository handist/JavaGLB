 ...
 * ratio of merge/emptied intra-bag operations to adjust the grain size of the
 * multithreaded global load balancer.
 * <p>
 * The current grain size is deemed too small if there are more than 1.3 times
 * the number of merge operations made on the intra-bag compared to the number
 * of times the intra-bag was emptied.
 * <p>
 * The grain size is deemed too large if less than 90% of the elapsed time was
 * spent with fewer than the maximum number of workers running.
 *
 * @author Patrick Finnerty
 *
 */
public class MergeEmptyTuner implements Tuner, Serializable {
    ...
    /**
     * Value of member {@link PlaceLogger#time} at the last index which counts
     * the accumulated time spent with the maximum number of workers.
     */
    ...
    /**
     * Contains the last interpretation that was made by the tuner on the value
     * of parameter N.
     * <p>
     ...
     * <li><em>0</em> if the last time the tuner was called the value was
     * neither judged to be too small nor too large, or if both indicators
     * indicated that it was too large and too small, or if the value of n was
     * modified during the last
     * {@link #tune(PlaceLogger, Configuration, GLBcomputer)} call
     * <li><em>1</em> if the last time the
     * {@link #tune(PlaceLogger, Configuration, GLBcomputer)} method was called,
    ...
    public long tune(PlaceLogger l, Configuration c, GLBcomputer g) {
        ...
        // CRITERIA GRAIN TOO SMALL : feed/empty > 1.3
        final boolean nTooSmall = feed * 10 > empty * 13;
        ...
        if (newValue != oldValue) {
            ...
            // operation does not actually change it. To avoid unnecessary logs
            // we
